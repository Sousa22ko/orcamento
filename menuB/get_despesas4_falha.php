...
<div id="totalMarcado">Total selecionado: R$ 0,00</div>
...
foreach ($despesas as $d) {
    ...
    /* -------- FECHA BALÃO -------- */
    //  Se a descrição atual for diferente da nova, fecha o balão anterior e exibe o total do balão
    //  atualiza as variáveis de controle para a nova descrição
    //  isso agrupa as despesas por descrição
    ...
    /* -------- ABRE BALÃO -------- */
    //  Se a descrição atual for diferente da nova, abre um novo balão  e exibe o CSS título
    //  atualiza as variáveis de controle para a nova descrição
    //  isso agrupa as despesas por descrição
    ...
    <div class='balao-linha'>
        ...
        <div class='valor'>R$ " . number_format($d['valor_casa'], 2, ',', '.') . "</div>
        <div>" . date('d/m/Y', strtotime($d['data'])) . "</div>
        <div><strong>" . parcela_func($d['parcela']) . "</strong></div>
        <div>{$d['nome_categoria']}</div>
        <div>{$statusPago}</div>
        <button class='btn-editar' data-id='{$d['id_despesa']}'>Editar</button>
    </div>";
...
document.querySelectorAll('.check').forEach(chk => {
    chk.addEventListener('change', e => {
        ...
        total += e.target.checked ? v : -v;
        document.getElementById('totalMarcado').innerText =
