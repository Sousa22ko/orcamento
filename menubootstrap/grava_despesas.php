...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {  
  ...
  $user_id = $_POST['user_id']; // ID do usuário vindo do formulário
...
////////////////////// INCLUIR AS DESPESAS - INICIO ////////////////////
    ...
    $sql = "INSERT INTO despesa (despesa, data, valor, valor_casa, ocorrencia, parcela, id_descricao, vencimento, pago, valido, obs, categoria,user_id) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    $stmtA->bind_param("ssddsssssssii", 
        $despesa,  
        $novadata, 
        $n_valor,  
        $valor_casa, 
...
if ($stmtA->execute()) {
    ...
    if (isset($_FILES['pdf']) && $_FILES['pdf']['error'] === UPLOAD_ERR_OK) {
        ...
        $stmtB = $conn->prepare("
            INSERT INTO tabela_imagens 
            (id_despesa_imagem,nome_imagem, tipo_imagem, tamanho_imagem, imagem) 
            VALUES (?, ?, ?, ?, ?)
        ");
        ...
        if (!$stmtB->execute()) {
            $conn->rollback();
            die("Erro ao salvar PDF: " . $stmtB->error);
        }

        $stmtB->close();
    }

    // 🔥 AGORA SIM: commit SEMPRE
    $conn->commit();
 ...
 $cont++;
    ...
    $conn->rollback();
    die("Erro ao inserir despesa: " . $stmtA->error);
}
